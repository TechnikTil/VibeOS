// MicroPython HAL implementation for VibeOS
...
// VibeOS special key codes (must match kernel/keyboard.c)
...
int mp_hal_stdin_rx_chr(void) {
    ...
    // Convert VibeOS special keys to VT100 escape sequences
    // Format: ESC [ <letter>
    if (c >= 0x100) {
...
mp_uint_t mp_hal_ticks_ms(void) {
    ...
    return mp_vibeos_api->get_uptime_ticks() * 10;
}

// Microseconds since boot
mp_uint_t mp_hal_ticks_us(void) {
    // VibeOS only has 10ms resolution, approximate
    return mp_vibeos_api->get_uptime_ticks() * 10000;
}
...
void mp_hal_delay_us(mp_uint_t us) {
    mp_vibeos_api->sleep_ms((us + 999) / 1000);
}
