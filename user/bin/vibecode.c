...
static void run_current_file(void) {
    ...
    if (syntax_mode == SYNTAX_C) {
        ...
        char out_path[64] = "/tmp/vibecode_out";
...
static void draw_welcome_screen(void) {
    ...
    // Subtitle
    buf_draw_string((win_w - 20 * 8) / 2, title_y + 24, "IDE for VibeOS", 0x00666666, 0x00F0F0F0);
...
int main(kapi_t *kapi, int argc, char **argv) {
    ...
    if (!api->window_create) {
        api->puts("vibecode: window API not available\n");
        return 1;
    }
    ...
    if (window_id < 0) {
        api->puts("vibecode: failed to create window\n");
        return 1;
    }

    win_buffer = api->window_get_buffer(window_id, &win_w, &win_h);
    if (!win_buffer) {
        api->puts("vibecode: failed to get window buffer\n");
